...
function login(email, password) {
    return function (dispatch) {
        ...
        window.fetch(index_1.authSettings.settings.apiUrl + index_1.authSettings.settings.signInPath, {
            ...
            if (result.ok) {
                var _a = [result.headers.get('uid'), result.headers.get('access-token'), result.headers.get('client')], uid = _a[0], token = _a[1], client = _a[2];
                ...
                })["catch"](function (error) { return dispatch(loadUserFailed()); });
            }
            else {
...
function validateToken(token) {
    return function (dispatch) {
        window.fetch(index_1.authSettings.settings.apiUrl + index_1.authSettings.settings.validateTokenPath, { headers: index_1.addAuthorizationHeader({}, token) }).then(function (result) {
            ...
            if (result.ok) {
                result.json().then(function (result) { return dispatch(loadUserSuccess(result.data)); });
            }
            else {
                console.log(result);
                dispatch(tokenDeleteSuccess());
